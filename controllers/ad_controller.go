...
import (
	...
	"github.com/imthewolverine/carpooling-backend/services"
)
...
func AddAd(c *gin.Context, adService *services.AdService) {
    ...
    ctx := context.Background()
    var ad services.Ad
    ad.Date = routeRequest.Date
    ad.Description = routeRequest.Description
    ad.Salary = routeRequest.Salary
    ad.School = routeRequest.School
    ad.SchoolAddress = routeRequest.SchoolAddress
    ad.Status = routeRequest.Status

    // Parse the Time field from string to time.Time
    parsedTime, err := time.Parse(time.RFC3339, routeRequest.Time)
    if err != nil {
...
func UpdateAdByID(c *gin.Context, adService *services.AdService) {
    ...
    ctx := context.Background()
    var route services.Ad
    route.Date = routeRequest.Date
    route.Description = routeRequest.Description
    route.School = routeRequest.School
    route.SchoolAddress = routeRequest.SchoolAddress
    ...
    route.Time = parsedTime

    // Convert ParentId to DocumentRef if provided
//    if route.ParentId != nil {
//        route.ParentId = adService.FirestoreClient.Collection("ads").Doc(route.ParentId)
//    }
//
    // Call the service to update the ad in Firestore
    if err := adService.UpdateAdByID(ctx, id, route); err != nil {
